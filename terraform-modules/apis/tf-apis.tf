...
resource "google_project_service" "enable_project_iam" {
  project = var.project_id
...
resource "time_sleep" "enable_api_iam_time_delay" {
  create_duration = "60s"

  depends_on = [google_project_service.enable_project_iam]
}
...
resource "time_sleep" "time_sleep_enable_api" {
  ...
  depends_on = [
		time_sleep.enable_api_iam_time_delay,
